#include "hearders/gpio.c"

struct gpiod_line_request *gpio_request_lines(const char *chip_path, unsigned int *offset, int num_lines, const char *consumer, char *in_out)
{
    ...
    if(strcmp(int_out, GPIO_DIRECTION_IN) == 0)
    {
        gpiod_line_settings_set_direction(settings, GPIOD_LINE_DIRECTION_INPUT);
    }else if(strcmp(int_out, GPIO_DIRECTION_OUT) == 0)
    {
